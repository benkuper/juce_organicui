...
public:
    ...
    public:
        ...
        {
            //discard if key down or up
            if(key.getKeyCode() == juce::KeyPress::upKey || key.getKeyCode() == juce::KeyPress::downKey) return false;
...
void ManagerUI<M, T, U>::setShowSearchBar(bool value)
{
    if (value)
    {
        ...
        searchBar->setFont(juce::FontOptions(10));
            ...
            if (T* item = bui->item)
            {
                ...
                else
                {
                    ...
                    if (T* newItem = this->manager->createItemFromData(data))
                    {
                        ...
                        if (Manager<T>* sourceManager = dynamic_cast<Manager<T> *>(item->parentContainer.get()))
                        {
