...
void EasingTimelineUIBase::paint(Graphics& g)
{
	...
	{
		Colour c = numDimensions == 1 ? TEXT_COLOR : dimensionColors[index];
		g.setColour(c);
...
void EasingTimelineUIBase::buildHitPath(int index)
{
	Path* drawPath = drawPaths[index];
	Path* hp = hitPaths[index];
	...
	{
		hp->startNewSubPath(firstPoints[0]);
		for (int i = 1; i < firstPoints.size(); i++)
		{
			if (!std::isnan(firstPoints[i].x)) hp->lineTo(firstPoints[i]);
		}
		for (int i = 0; i < secondPoints.size(); i++)
		{
			if (!std::isnan(firstPoints[i].x)) hp->lineTo(secondPoints[i]);
		}
		hp->closeSubPath();
	}
}
...
bool EasingTimelineUIBase::hitTest(int tx, int ty)
{
	for (auto& hitPath : hitPaths) if (hitPath->contains((float)tx, (float)ty)) return true;
	return false;
}
