...
public:
	...
	enum class RangeMode { RGBA, HSVA };
	RangeMode getRangeMode() const { return rangeMode; }
	void setBounds(RangeMode mode, float _minR, float _minG, float _minB, float _minA, float _maxR, float _maxG, float _maxB, float _maxA);
	void clearRange() override;
	bool hasRange() const override;

	void setNormalizedValue(const juce::var& normalizedValue, bool silentSet = false, bool force = false) override;
	juce::var getNormalizedValue() const override;
...
private:
	RangeMode rangeMode = RangeMode::RGBA;
};
