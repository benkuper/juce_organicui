	...
	if (e.mods.isLeftButtonDown())
	{
		if (canBeCollapsed())
		{
			if ((e.eventComponent == this && e.getMouseDownPosition().y < headerHeight && !dragRect.contains(e.getMouseDownPosition())) || (isRoot && e.eventComponent == this && e.getMouseDownY() < headerHeight))
			{
				...
				{
					if (!e.mouseWasDraggedSinceMouseDown()) setCollapsed(!container->editorIsCollapsed);
				}
			}
		}
	}
...
void GenericControllableContainerEditor::showContextMenu()
{
	...
	if (ControllableUI::showDashboardOption)
	{
		...
		DashboardManager::getInstance()->callFunctionOnItems([&dashboardMenu, &index](auto di)
			{
	...
	p.showMenuAsync(PopupMenu::Options(), [this](int result)
		{
			if (result != 0)
			{
				...
				default:
					if (result >= 10000)
					{
						DashboardManager::getInstance()->getItemAt(result - 10000)->itemManager.addItem(this->container->createDashboardItem());
					}
					else
					{
